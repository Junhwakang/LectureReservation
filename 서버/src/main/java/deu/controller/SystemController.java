    ...
    public Object handle(Object request) {
        try {
            ...
            // 예약 관리 컨트롤러 (기존 방식 유지)
                ...
                // ⭐ 지연 초기화: 예약 관리 요청이 올 때만 초기화
                if (reservationManagementController == null) {
                    try {
                        reservationManagementController = ReservationManagementController.getInstance();
                    } catch (Exception e) {
                        System.err.println("[SystemController] ReservationManagementController 초기화 실패: " + e.getMessage());
                        return new BasicResponse("500", "예약 관리 기능 초기화 실패");
                    }
                }
                return switch (r.command) {

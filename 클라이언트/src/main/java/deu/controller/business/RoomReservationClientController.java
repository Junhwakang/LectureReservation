...
public class RoomReservationClientController {
    ...
    public BasicResponse addRoomReservation(RoomReservationRequest roomReservationRequest) {
        try (
            ...
            if (res instanceof BasicResponse r) {
                ...
                return new BasicResponse("500", "예기치 않은 서버 응답 타입: "
                        + (res != null ? res.getClass().getName() : "null"));
            }

        } catch (java.io.EOFException e) {
            e.printStackTrace();
            return new BasicResponse("500", "서버와의 연결이 끊어졌습니다. (예약 처리 중)");
        } catch (Exception e) {
            e.printStackTrace();
            return new BasicResponse("500", "예약 처리 중 통신 오류: " + e.toString()); // ★ toString() 사용
        }
    }
    ...
    public BasicResponse userRoomReservationList(String number) {
        try (
            ...
            System.out.println("서버 통신 실패: " + e);
            return new BasicResponse("500", "서버 통신 실패: " + e); // ★ null 대신 에러 응답
        }
    }
